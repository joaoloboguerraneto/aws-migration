...
**Route 53** Configurar health checks no endpoint on-premises. Se o health check falhar 3 vezes seguidas, o DNS faz failover automático para o ALB na AWS.

### Como funciona o failover

1. Route 53 detecta que o on-prem está em baixo
2. DNS TTL expira, tráfego redireccionado para AWS
3. RDS replica é promovida a standalone
4. Pods no EKS já estão a correr, começam a servir
5. Cluster Autoscaler escala nodes conforme a carga

Downtime total para o utilizador
...
O on-prem fica como rollback durante 48 horas se algo estiver errado, voltamos com uma mudança de DNS.
